...
int main(int argc, char** argv)
{
    ...
    for (const auto& data : campaigns) {
        ...
        for (const auto& model : data.history()) {
            ...
            if (days < 0)
                continue;
            margin_history.emplace_back(days, model.metamargin());
        }
    ...
    while (true) {
        std::vector<std::string> strings = { std::to_string(days_left) };
        for (auto& pair : iters) {
            ...
            strings.emplace_back(std::to_string(pair.second->second));
            pair.second++;
        }
        if (done)
            break;
        lines.emplace_back(ke::Join(strings, ","));
        days_left++;
    }
